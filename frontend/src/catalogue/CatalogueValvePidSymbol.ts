import { ValveCatalogueModel } from './CatalogueValveTypes';

export interface ValvePidSymbol {
  id: string;
  sourceModelId: string;
  viewBox: string;
  ports: Readonly<Record<'inlet' | 'outlet', { x: number; y: number; direction: readonly [number, number] }>>;
  svg: string;
}

const basePorts = {
  inlet: { x: 0, y: 30, direction: [-1, 0] as const },
  outlet: { x: 120, y: 30, direction: [1, 0] as const },
};

const bodyForClass = (model: ValveCatalogueModel): string => {
...
export const createCatalogueValvePidSymbol = (model: ValveCatalogueModel): ValvePidSymbol => {
